  ...
  return (
    <header className="sticky top-0 z-50 bg-white/70 backdrop-blur-lg shadow-sm">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2 md:py-3">
        ...
        {/* Burger */}
        <button className="inline-flex items-center sm:hidden" onClick={() => setOpen(!open)}>
          {open ? <X size={28} /> : <Menu size={28} />}
        </button>

        {/* Links */}
        <nav
          className={`${open ? "block" : "hidden"} absolute left-0 top-full w-full bg-white sm:static sm:block sm:w-auto sm:bg-transparent`}
        >
